...
class Boot{
	...
	function payload($lang,$code){
		...
		if($lang=='java')
			...
			$payload = '{"language":"c","command":"clang main.c && ./a.out -lm '.$input.'", "files": [{"name": "main.c", "content": '.$code.'}]}';
		else if($lang =='clang' && $this->get('c')==0)
			$payload = '{"language":"c","command":"clang++ main.cpp && ./a.out -lm'.$input.'", "files": [{"name": "main.cpp", "content": '.$code.'}]}';
		else if($lang =='python')
